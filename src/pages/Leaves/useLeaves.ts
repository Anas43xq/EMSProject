...
export function useLeaves() {
  ...
  // Subscribe to realtime updates for leaves (so HR sees new requests instantly)
  useEffect(() => {
    if (!user) return;
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          // Reload leaves when any change happens
          loadLeaves();
        }
      )
      .subscribe();
  ...
  const handleApplyLeave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create in-app database notifications for HR/Admin users
